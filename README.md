# Hi 👋 I'm Drishika Chauhan  

## 👩‍💻 About Me

- Associate Consultant in a **Fintech organization**
- Interested in backend architecture & low-level system design
- Always improving core engineering fundamentals

---

## ⚙️ Tech Stack

### 🖥 Backend
`Java` • `Spring Boot` • `PL/SQL` • `REST APIs` • `WebLogic`

### 🎨 Frontend
`React.js` • `HTML5` • `CSS3` • `Bootstrap`

### 🔧 Tools
`Git` • `GitHub`

---

## 📚 Currently Exploring

- Advanced Spring
- Low-Level Design
- Data Structures & Algorithms
- Payment Systems Architecture

---

## 📫 Connect With Me

📧 drishika.dcs16@gmail.com  
💼 [LinkedIn](https://www.linkedin.com/in/drishika-chauhan-647254206/)  
✍️ [Blogs](https://drishika-dcs16.medium.com/)
